...
# ==============================================================================
# KEENETIC FIREWALL STATS v2.3.3 (STABLE - RESTORED LOG DETAILS)
# Features: Dual Main Tabs, Top 10 Days (Bottom), Detailed System Logs
# Change Log: 
#   v2.3.3: Restored "NEW_RECORDS" counter for detailed logging.
# ==============================================================================
...
init_db() {
    ...
    HAS_IP=$(sqlite3 "$DB_FILE" "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='ip_drops';")
    [ "$HAS_IP" -eq 0 ] && sqlite3 "$DB_FILE" "CREATE TABLE ip_drops (timestamp INTEGER, ip TEXT, count INTEGER); CREATE INDEX idx_ip_ts ON ip_drops(timestamp); CREATE INDEX idx_ip_ip ON ip_drops(ip);"
    HAS_CACHE=$(sqlite3 "$DB_FILE" "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='ip_info';")
    [ "$HAS_CACHE" -eq 0 ] && sqlite3 "$DB_FILE" "CREATE TABLE ip_info (ip TEXT PRIMARY KEY, country TEXT, risk INTEGER, domain TEXT, updated INTEGER);"
}
init_db
...
for LIST in $LISTS; do
    ...
    if [ "$LIST" = "FirewallBlock" ]; then 
        ...
        LIST_MAIN_DIFF="="; [ -f "$DIFF_FILE_MAIN" ] && LIST_MAIN_DIFF=$(cat "$DIFF_FILE_MAIN")
    fi
    if [ "$LIST" = "VPNBlock" ]; then 
        SIZE_VPN=$(ipset list "$LIST" | grep -cE '^[0-9]')
        LIST_VPN_DIFF="+0"; [ -f "$DIFF_FILE_VPN" ] && LIST_VPN_DIFF=$(cat "$DIFF_FILE_VPN")
    fi
done
...
# Top 10 Days
gen_top_days() {
    QUERY="SELECT strftime('%d-%m-%Y', timestamp, 'unixepoch', 'localtime') as day, SUM(CASE WHEN list_name='FirewallBlock' THEN count ELSE 0 END), SUM(CASE WHEN list_name='VPNBlock' THEN count ELSE 0 END), SUM(count) as total FROM drops GROUP BY day ORDER BY total DESC LIMIT 10;"
    sqlite3 -separator "|" "$DB_FILE" "$QUERY" | awk -F'|' '{print "<tr><td><b>"$1"</b></td><td class=\"num col-main\">+"$2"</td><td class=\"num col-vpn\">+"$3"</td><td class=\"num\">+"$4"</td></tr>"}'
}
...
'
    for LINE in $TOP_LIST; do
        IP=$(echo "$LINE" | cut -d'|' -f1); COUNT=$(echo "$LINE" | cut -d'|' -f2); CLEAN_IP=$(echo "$IP" | cut -d'/' -f1)
        CACHE=$(sqlite3 -separator "|" "$DB_FILE" "SELECT country, risk, domain FROM ip_info WHERE ip='$CLEAN_IP';")
        
        if [ -n "$CACHE" ]; then
            COUNTRY=$(echo "$CACHE" | cut -d'|' -f1); SCORE=$(echo "$CACHE" | cut -d'|' -f2); DOMAIN=$(echo "$CACHE" | cut -d'|' -f3)
        else
            JSON=$(curl -s -m 3 -G https://api.abuseipdb.com/api/v2/check --data-urlencode "ipAddress=$CLEAN_IP" -d maxAgeInDays=90 -H "Key: $ABUSEIPDB_KEY" -H "Accept: application/json" || echo "")
            COUNTRY=$(echo "$JSON" | grep -o '"countryCode":"[^"]*"' | cut -d'"' -f4); SCORE=$(echo "$JSON" | grep -o '"abuseConfidenceScore":[0-9]*' | cut -d':' -f2); DOMAIN=$(echo "$JSON" | grep -o '"domain":"[^"]*"' | cut -d'"' -f4)
            [ -z "$SCORE" ] && SCORE=0
            if [ -n "$COUNTRY" ]; then sqlite3 "$DB_FILE" "INSERT OR REPLACE INTO ip_info (ip, country, risk, domain, updated) VALUES ('$CLEAN_IP', '$COUNTRY', $SCORE, '$DOMAIN', $NOW);"; fi
        fi
        
        if [ "$SCORE" -ge 50 ]; then META="<span style='color:var(--red); font-weight:bold;'>Risk: ${SCORE}%</span> - $COUNTRY<br><small>$DOMAIN</small>"
        else META="<span style='color:var(--green);'>Risk: ${SCORE}%</span> - $COUNTRY<br><small>$DOMAIN</small>"; fi
        echo "<tr><td><span class='badge bg-danger'>$COUNT</span></td><td><a href='https://www.abuseipdb.com/check/$CLEAN_IP' target='_blank'>$IP</a> <span class='badge $BADGE'>$TYPE</span></td><td class='meta'>$META</td></tr>"
    done
    unset IFS
}
...
# --- JSON EXPORT ---
DATE_UPDATE=$($DATE_CMD "+%d-%m-%Y %H:%M:%S"); UP_SECONDS=$(cut -d. -f1 /proc/uptime); DAYS=$((UP_SECONDS / 86400)); HOURS=$(( (UP_SECONDS % 86400) / 3600 )); UPTIME="${DAYS}d ${HOURS}h"
...
<head>
    ...
    <style>
        :root { --bg: #f4f7f6; --card: #ffffff; --text: #333; --muted: #6c757d; --border: #e9ecef; --th-bg: #f8f9fa; --red: #dc3545; --orange: #fd7e14; --blue: #0d6efd; --green: #198754; --gray: #6c757d; --shadow: rgba(0,0,0,0.03); --diff-pos: rgba(25, 135, 84, 0.1); --diff-neg: rgba(220, 53, 69, 0.1); --diff-eq: rgba(108, 117, 125, 0.1); }
        ...
        .status-bar { display: flex; justify-content: space-between; align-items: center; background: var(--card); padding: 15px 20px; border-radius: 10px; box-shadow: 0 2px 10px var(--shadow); border: 1px solid var(--border); margin-bottom: 25px; }
        @media (max-width: 600px) { .status-bar { flex-direction: column; text-align: center; gap: 15px; } }
        .btn-refresh { background-color: var(--blue); color: #fff; border: none; padding: 10px 20px; border-radius: 6px; font-size: 13px; font-weight: 600; text-decoration: none; cursor: pointer; transition: background-color 0.2s, transform 0.1s; display: inline-block; }
...
<body>
    ...
    <div id="view_history" class="view-section">
        ...
        <div class="tables-grid">
            <div style="grid-column: 1 / -1;"><div class="section-title"><span>🏆 Top 10 Days by Volume (All Time)</span></div><div class="table-container"><table><thead><tr><th>Date</th><th class="num">Net</th><th class="num">VPN</th><th class="num">Total</th></tr></thead><tbody id="tb_top_days"></tbody></table></div></div>
        </div>
    </div>

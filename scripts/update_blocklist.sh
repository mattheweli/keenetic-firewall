...
# ==============================================================================
# BLOCKLIST UPDATER v1.3.4
# Features: Multi-Source, Deduplication, Persistent Stats, FW Hook
# Change Log: v1.3.4 - Added filter for Link-Local addresses (169.254.x.x)
# ==============================================================================
...
# 3. Logic Branch: UPDATE vs RESTORE
if [ "$DOWNLOAD_COUNT" -gt 0 ] && [ -s "$RAW_FILE" ]; then
    ...
    # Load data into temp set WITH DEDUPLICATION & FILTERS
    # Added |169.254. to exclude link-local garbage
    cat "$RAW_FILE" \
        | grep -vE "^#|^$|0.0.0.0|127.0.0.1|10.0.0.0|192.168.|172.16.|169.254." \
        | sort -u \
...
if ipset list -n "$IPSET_VPN" >/dev/null 2>&1; then
    ...
    VPN_DELTA=$((VPN_END - VPN_START))
    
    if [ "$VPN_DELTA" -ge 0 ]; then VPN_DIFF_STR="+$VPN_DELTA"; else VPN_DIFF_STR="$VPN_DELTA"; fi
    echo "$VPN_DIFF_STR" > "$DIFF_FILE_VPN"
    
    if [ "$CLEAN_COUNT" -gt 0 ]; then
        ipset list "$IPSET_VPN" | grep -E '^[0-9]' > "$VPN_FILE"
        logger -t "$LOG_TAG" "Cleaned $CLEAN_COUNT duplicate IPs from VPN list. (Change: $VPN_DIFF_STR)"
    else
        logger -t "$LOG_TAG" "VPN list checked. No duplicates found. (Change: $VPN_DIFF_STR)"
    fi
else
    echo "+0" > "$DIFF_FILE_VPN"
fi
